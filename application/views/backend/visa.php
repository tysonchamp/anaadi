    ...
    <section class="section dashboard">
        <div class="col-lg-12 p-0">
          <div class="row">
            <div class="card">
              <div class="card-body">
                <h5 class="card-title">Visa <button type="button" class="new-visa btn btn-md btn-primary float-right">Add <i class="bi bi-plus-circle ms-1"></i></button></h5>
...
<div class="modal fade" id="add-visa" tabindex="-1" data-bs-backdrop="false">
  <div class="modal-dialog">
    <div class="modal-content">
      <form id="addvisa" action="<?=base_url('admin/Visa/save_record')?>" method="POST">
          ...
          <div class="modal-body">
              <div class="col-md-10 mx-auto">
                <label for="title" class="form-label">Visa Title</label>
                <input type="text" maxlength="255" autocomplete="off" class="form-control" name="title" id="title" value="" required>
              </div>
          </div>
          <div class="modal-footer">
              <button class="btn btn-primary" id="submitvisa" type="button">Submit</button>
...
<script type="text/javascript">
  $(document).ready(function(){
    var module_url = '<?=base_url('/admin/Visa')?>';

    $(".new-visa").click(function (event) {
        event.preventDefault();
        $('#addvisa')[0].reset();
        $('#add-visa input[name="record_id"]').val('');
        $("#add-visa").modal('show');
    });
    ...
    $(".edit-record").click(function (event) {
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                {
                  $('#add-visa').modal('show');
                  $('#add-visa input[name="record_id"]').val(d.record.id);
                  $('#add-visa input[name="title"]').val(d.record.title);
                }
                else
                {
    ...
    $("#submitvisa").click(function (event) {
        event.preventDefault();
        let form = $("#addvisa");
        let mbody = $("#addvisa .modal-body");
        let url = form.attr('action');
        var title = $("#addvisa input[name='title']").val().trim();

        if(title == "") {
            mbody.prepend("<div class='alert alert-danger mt-1 mb-2'>Please enter visa title</div>");
            $("#addvisa input[name='title']").focus();
            return false;
        }
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                if( d.error == 1 )
                {
                    ...
                    $("#submitvisa").prop('disabled', false);
                    $("#submitvisa").html("Submit");
                }
                else
                {
                    mbody.append("<div class='alert alert-success mt-1 mb-0'>"+d.success_message+"</div>");
                    $("#submitvisa").html("Success");
                    setTimeout(function(){
