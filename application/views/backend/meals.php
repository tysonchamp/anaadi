    ...
    <section class="section dashboard">
        <div class="col-lg-12 p-0">
          <div class="row">
            <div class="card">
              <div class="card-body">
                <h5 class="card-title">Meals <button type="button" class="new-meal btn btn-md btn-primary float-right">Add <i class="bi bi-plus-circle ms-1"></i></button></h5>
...
<div class="modal fade" id="add-meal" tabindex="-1" data-bs-backdrop="false">
  <div class="modal-dialog">
    <div class="modal-content">
      <form id="addmeal" action="<?=base_url('admin/Meals/save_record')?>" method="POST">
          ...
          <div class="modal-footer">
              <button class="btn btn-primary" id="submitmeal" type="button">Submit</button>
...
<script type="text/javascript">
  $(document).ready(function(){
    var module_url = '<?=base_url('/admin/Meals')?>';

    $(".new-meal").click(function (event) {
        event.preventDefault();
        $('#addmeal')[0].reset();
        $('#add-meal input[name="record_id"]').val('');
        $("#add-meal").modal('show');
    });
    ...
    $(".edit-record").click(function (event) {
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                {
                  $('#add-meal').modal('show');
                  $('#add-meal input[name="record_id"]').val(d.record.id);
                  $('#add-meal input[name="name"]').val(d.record.name);
                }
                else
                {
    ...
    $("#submitmeal").click(function (event) {
        event.preventDefault();
        let form = $("#addmeal");
        let mbody = $("#addmeal .modal-body");
        let url = form.attr('action');
        var name = $("#addmeal input[name='name']").val().trim();

        if(name == "") {
            mbody.prepend("<div class='alert alert-danger mt-1 mb-2'>Please enter meal name</div>");
            $("#addmeal input[name='name']").focus();
            return false;
        }
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                if( d.error == 1 )
                {
                    ...
                    $("#submitmeal").prop('disabled', false);
                    $("#submitmeal").html("Submit");
                }
                else
                {
                    mbody.append("<div class='alert alert-success mt-1 mb-0'>"+d.success_message+"</div>");
                    $("#submitmeal").html("Success");
                    setTimeout(function(){
