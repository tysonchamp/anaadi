    ...
    <section class="section dashboard">
        <div class="col-lg-12 p-0">
          <div class="row">
            <div class="card">
              <div class="card-body">
                <h5 class="card-title">Articles <button type="button" class="new-article btn btn-md btn-primary float-right">Add <i class="bi bi-plus-circle ms-1"></i></button></h5>
...
<div class="modal fade" id="add-article" tabindex="-1" data-bs-backdrop="false">
  <div class="modal-dialog">
    <div class="modal-content">
      <form id="addarticle" action="<?=base_url('admin/Articles/save_record')?>" method="POST">
          ...
          <div class="modal-footer">
              <button class="btn btn-primary" id="submitarticle" type="button">Submit</button>
...
<script type="text/javascript">
  $(document).ready(function(){
    var module_url = '<?=base_url('/admin/Articles')?>';

    $(".new-article").click(function (event) {
        event.preventDefault();
        $('#addarticle')[0].reset();
        $('#add-article input[name="record_id"]').val('');
    ...
    $(".edit-record").click(function (event) {
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                {
                  $('#add-article').modal('show');
                  $('#add-article input[name="record_id"]').val(d.record.id);
                  $('#add-article input[name="name"]').val(d.record.name);
                }
                else
                {
    ...
    $("#submitarticle").click(function (event) {
        event.preventDefault();
        let form = $("#addarticle");
        let mbody = $("#addarticle .modal-body");
        let url = form.attr('action');
        var name = $("#addarticle input[name='name']").val().trim();

        if(name == "") {
            mbody.prepend("<div class='alert alert-danger mt-1 mb-2'>Please enter article name</div>");
            $("#addarticle input[name='name']").focus();
            return false;
        }
