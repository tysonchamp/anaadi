    ...
    <section class="section dashboard">
        <div class="col-lg-12 p-0">
          <div class="row">
            <div class="card">
              <div class="card-body">
                <h5 class="card-title">Activities <button type="button" class="new-activity btn btn-md btn-primary float-right">Add <i class="bi bi-plus-circle ms-1"></i></button></h5>
...
<div class="modal fade" id="add-activity" tabindex="-1" data-bs-backdrop="false">
  <div class="modal-dialog">
    <div class="modal-content">
      <form id="addactivity" action="<?=base_url('admin/Activities/save_record')?>" method="POST">
          ...
          <div class="modal-footer">
              <button class="btn btn-primary" id="submitactivity" type="button">Submit</button>
...
<script type="text/javascript">
  $(document).ready(function(){
    var module_url = '<?=base_url('/admin/Activities')?>';

    $(".new-activity").click(function (event) {
        event.preventDefault();
        $('#addactivity')[0].reset();
        $('#add-activity input[name="record_id"]').val('');
    ...
    $(".edit-record").click(function (event) {
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                {
                  $('#add-activity').modal('show');
                  $('#add-activity input[name="record_id"]').val(d.record.id);
                  $('#add-activity input[name="name"]').val(d.record.name);
                }
                else
                {
    ...
    $("#submitactivity").click(function (event) {
        event.preventDefault();
        let form = $("#addactivity");
        let mbody = $("#addactivity .modal-body");
        let url = form.attr('action');
        var name = $("#addactivity input[name='name']").val().trim();

        if(name == "") {
            mbody.prepend("<div class='alert alert-danger mt-1 mb-2'>Please enter activity name</div>");
            $("#addactivity input[name='name']").focus();
            return false;
        }
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                if( d.error == 1 )
                {
                    ...
                    $("#submitactivity").prop('disabled', false);
                    $("#submitactivity").html("Submit");
                }
                else
                {
                    mbody.append("<div class='alert alert-success mt-1 mb-0'>"+d.success_message+"</div>");
                    $("#submitactivity").html("Success");
                    setTimeout(function(){
