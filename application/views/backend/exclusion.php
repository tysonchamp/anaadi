    ...
    <section class="section dashboard">
        <div class="col-lg-12 p-0">
          <div class="row">
            <div class="card">
              <div class="card-body">
                <h5 class="card-title">Exclusions <button type="button" class="new-exclusion btn btn-md btn-primary float-right">Add <i class="bi bi-plus-circle ms-1"></i></button></h5>
...
<div class="modal fade" id="add-exclusion" tabindex="-1" data-bs-backdrop="false">
  <div class="modal-dialog">
    <div class="modal-content">
      <form id="addexclusion" action="<?=base_url('admin/Exclusion/save_record')?>" method="POST">
          ...
          <div class="modal-body">
              <div class="col-md-10 mx-auto">
                <label for="name" class="form-label">Exclusion Name</label>
                <input type="text" maxlength="128" autocomplete="off" class="form-control" name="name" id="name" value="" required>
              </div>
          </div>
          <div class="modal-footer">
              <button class="btn btn-primary" id="submitexclusion" type="button">Submit</button>
...
<script type="text/javascript">
  $(document).ready(function(){
    var module_url = '<?=base_url('/admin/Exclusion')?>';

    $(".new-exclusion").click(function (event) {
        event.preventDefault();
        $('#addexclusion')[0].reset();
        $('#add-exclusion input[name="record_id"]').val('');
        $("#add-exclusion").modal('show');
    });
    ...
    $(".edit-record").click(function (event) {
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                {
                  $('#add-exclusion').modal('show');
                  $('#add-exclusion input[name="record_id"]').val(d.record.id);
                  $('#add-exclusion input[name="name"]').val(d.record.name);
                }
                else
                {
    ...
    $("#submitexclusion").click(function (event) {
        event.preventDefault();
        let form = $("#addexclusion");
        let mbody = $("#addexclusion .modal-body");
        let url = form.attr('action');
        var name = $("#addexclusion input[name='name']").val().trim();

        if(name == "") {
            mbody.prepend("<div class='alert alert-danger mt-1 mb-2'>Please enter exclusion name</div>");
            $("#addexclusion input[name='name']").focus();
            return false;
        }
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                if( d.error == 1 )
                {
                    ...
                    $("#submitexclusion").prop('disabled', false);
                    $("#submitexclusion").html("Submit");
                }
                else
                {
                    mbody.append("<div class='alert alert-success mt-1 mb-0'>"+d.success_message+"</div>");
                    $("#submitexclusion").html("Success");
                    setTimeout(function(){
