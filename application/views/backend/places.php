    ...
    <section class="section dashboard">
        <div class="col-lg-12 p-0">
          <div class="row">
            <div class="card">
              <div class="card-body">
                <h5 class="card-title">Places <button type="button" class="new-place btn btn-md btn-primary float-right">Add <i class="bi bi-plus-circle ms-1"></i></button></h5>
                ...
                <table class="table datatable table-hover table-sm">
                  ...
                  <tbody>
                    ...
                    <tr>
                      ...
                      <td><?=htmlspecialchars($row['place_type'])?></td>
                      <td>
...
<div class="modal fade" id="add-place" tabindex="-1" data-bs-backdrop="false">
  <div class="modal-dialog">
    <div class="modal-content">
      <form id="addplace" action="<?=base_url('admin/Places/save_record')?>" method="POST">
          ...
          <div class="modal-body">
              ...
              <div class="col-md-10 mx-auto">
                ...
                <select name="place_type" id="place_type" class="form-select" required>
                  ...
                  <option value="deprture">Departure</option>
                  <option value="arrival">Arrival</option>
                </select>
              </div>
          </div>
          <div class="modal-footer">
              <button class="btn btn-primary" id="submitplace" type="button">Submit</button>
...
<script type="text/javascript">
  $(document).ready(function(){
    var module_url = '<?=base_url('/admin/Places')?>';

    $(".new-place").click(function (event) {
        event.preventDefault();
        $('#addplace')[0].reset();
        $('#add-place input[name="record_id"]').val('');
    ...
    $(".edit-record").click(function (event) {
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                {
                  $('#add-place').modal('show');
                  $('#add-place input[name="record_id"]').val(d.record.id);
                  $('#add-place input[name="name"]').val(d.record.name);
                  $('#add-place select[name="place_type"]').val(d.record.place_type);
                }
                else
                {
    ...
    $("#submitplace").click(function (event) {
        event.preventDefault();
        let form = $("#addplace");
        let mbody = $("#addplace .modal-body");
        let url = form.attr('action');
        var name = $("#addplace input[name='name']").val().trim();
        var place_type = $("#addplace select[name='place_type']").val();

        if(name == "") {
            mbody.prepend("<div class='alert alert-danger mt-1 mb-2'>Please enter place name</div>");
            $("#addplace input[name='name']").focus();
            return false;
        }
        if(place_type == "") {
            mbody.prepend("<div class='alert alert-danger mt-1 mb-2'>Please select place type</div>");
            $("#addplace select[name='place_type']").focus();
            return false;
        }
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                if( d.error == 1 )
                {
                    ...
                    $("#submitplace").prop('disabled', false);
                    $("#submitplace").html("Submit");
                }
                else
                {
                    mbody.append("<div class='alert alert-success mt-1 mb-0'>"+d.success_message+"</div>");
                    $("#submitplace").html("Success");
                    setTimeout(function(){
